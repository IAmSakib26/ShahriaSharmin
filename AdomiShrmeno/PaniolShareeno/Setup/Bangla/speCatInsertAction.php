...
<title>Special Category :: Insert (Bangla)</title>
...
<div class="page-title"><h3 class="title">Dashboard / Special Category :: Insert (Bangla)</h3></div>
<div class="row">
	<div class="col-sm-12 DPaddingLR">
		<div class="panel panel-default">
			<div class="panel-heading">
				<h3 class="panel-title">Special Category :: Insert (Bangla)</h3>
			</div>
			<div class="panel-body">
				...
				if(isset($_POST["btnSubmit"])){
					...
					$qInsert="INSERT INTO bn_bas_specialcategory(SpecialCategoryName, Slug, EndNote, Remarks, Priority, ShowData, ImagePathIcon, ImagePathMenu, ImagePathCoverHome, ImagePathCoverInner)
					...
					if(mysqli_query($connEMM, $qInsert)){
						...
						VALUES('".$_SESSION["sessUserName"]."', 1, 'bn_bas_specialcategory', '".$_SERVER["REMOTE_ADDR"]."', '".$_SERVER["REQUEST_URI"]."', '".fAuTrail($qInsert)."', '".$dtDateTime."')";
								...
								while($fileRen=readdir($dir)){
									if($fileRen==$sImagePathIcon){
										...
										if($iFlag==1){
											...
											$qUpdate="UPDATE bn_bas_specialcategory SET ImagePathIcon='".$sNewName."' WHERE SpecialCategoryID=".$iThisContentID;
											//echo $qUpdate."<br>";
							...
							if(move_uploaded_file($_FILES["txtImageMenu"]["tmp_name"], $UploadCategory.$_FILES["txtImageMenu"]["name"])){
								...
								while($fileRen=readdir($dir)){
									if($fileRen==$sImagePathMenu){
										...
										if($iFlag==1){
											//If Rename was done properly -- Update the new uploaded file name information into the database
											$qUpdate="UPDATE bn_bas_specialcategory SET ImagePathMenu='".$sNewName."' WHERE SpecialCategoryID=".$iThisContentID;
											//echo $qUpdate."<br>";
							...
							if(move_uploaded_file($_FILES["txtImageCoverHome"]["tmp_name"], $UploadCategory.$_FILES["txtImageCoverHome"]["name"])){
								...
								while($fileRen=readdir($dir)){
									if($fileRen==$sImagePathCoverHome){
										...
										if($iFlag==1){
											//If Rename was done properly -- Update the new uploaded file name information into the database
											$qUpdate="UPDATE bn_bas_specialcategory SET ImagePathCoverHome='".$sNewName."' WHERE SpecialCategoryID=".$iThisContentID;
											//echo $qUpdate."<br>";
							...
							if(move_uploaded_file($_FILES["txtImageCoverInner"]["tmp_name"], $UploadCategory.$_FILES["txtImageCoverInner"]["name"])){
								...
								while($fileRen=readdir($dir)){
									if($fileRen==$sImagePathCoverInner){
										...
										if($iFlag==1){
											//If Rename was done properly -- Update the new uploaded file name information into the database
											$qUpdate="UPDATE bn_bas_specialcategory SET ImagePathCoverInner='".$sNewName."' WHERE SpecialCategoryID=".$iThisContentID;
											//echo $qUpdate."<br>";
