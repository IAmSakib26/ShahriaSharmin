		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				if(isset($_POST["btnSubmit"])){
					...
					$qInsert="INSERT INTO en_bas_category(CategoryName, Slug, EndNote, Remarks, Priority, ImagePathIcon, ImagePathMenu, ImagePathCoverHome, ImagePathCoverInner, ShowData)
					...
					if(mysqli_query($connEMM, $qInsert)){
						...
						VALUES('".$_SESSION["sessUserName"]."', 1, 'en_bas_category', '".$_SERVER["REMOTE_ADDR"]."', '".$_SERVER["REQUEST_URI"]."', '".fAuTrail($qInsert)."', '".$dtDateTime."')";
								...
								while($fileRen=readdir($dir)){
									if($fileRen==$sImagePathIcon){
										...
										if($iFlag==1){
											...
											$qUpdate="UPDATE en_bas_category SET ImagePathIcon='".$sNewName."' WHERE CategoryID=".$iThisContentID;
											//echo $qUpdate."<br>";
							...
							if(move_uploaded_file($_FILES["txtImageMenu"]["tmp_name"], $UploadCategory.$_FILES["txtImageMenu"]["name"])){
								...
								while($fileRen=readdir($dir)){
									if($fileRen==$sImagePathMenu){
										...
										if($iFlag==1){
											//If Rename was done properly -- Update the new uploaded file name information into the database
											$qUpdate="UPDATE en_bas_category SET ImagePathMenu='".$sNewName."' WHERE CategoryID=".$iThisContentID;
											//echo $qUpdate."<br>";
							...
							if(move_uploaded_file($_FILES["txtImageCoverHome"]["tmp_name"], $UploadCategory.$_FILES["txtImageCoverHome"]["name"])){
								...
								while($fileRen=readdir($dir)){
									if($fileRen==$sImagePathCoverHome){
										...
										if($iFlag==1){
											//If Rename was done properly -- Update the new uploaded file name information into the database
											$qUpdate="UPDATE en_bas_category SET ImagePathCoverHome='".$sNewName."' WHERE CategoryID=".$iThisContentID;
											//echo $qUpdate."<br>";
							...
							if(move_uploaded_file($_FILES["txtImageCoverInner"]["tmp_name"], $UploadCategory.$_FILES["txtImageCoverInner"]["name"])){
								...
								while($fileRen=readdir($dir)){
									if($fileRen==$sImagePathCoverInner){
										...
										if($iFlag==1){
											//If Rename was done properly -- Update the new uploaded file name information into the database
											$qUpdate="UPDATE en_bas_category SET ImagePathCoverInner='".$sNewName."' WHERE CategoryID=".$iThisContentID;
											//echo $qUpdate."<br>";
