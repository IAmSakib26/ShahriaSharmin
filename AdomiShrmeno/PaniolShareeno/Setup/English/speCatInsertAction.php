...
<title>Special Category :: Insert (English)</title>
...
<div class="page-title"><h3 class="title">Dashboard / Special Category :: Insert (English)</h3></div>
<div class="row">
	<div class="col-sm-12 DPaddingLR">
		<div class="panel panel-default">
			<div class="panel-heading">
				<h3 class="panel-title">Special Category :: Insert (English)</h3>
			</div>
			<div class="panel-body">
				<?php
				if(isset($_POST["btnSubmit"])){
					$iUpdateID=1;$iPriority=1;$iShow=1;
					...
					$qInsert="INSERT INTO en_bas_specialcategory(SpecialCategoryName, Slug, EndNote, Remarks, Priority, ShowData, ImagePathIcon, ImagePathMenu, ImagePathCoverHome, ImagePathCoverInner)
					VALUES('".$sSpecialCategoryName."', '".$sSlug."', '".$sEndNote."', '".$sRemarks."', ".$iPriority.", ".$iShow.", '".$sImagePathIcon."', '".$sImagePathMenu."', '".$sImagePathCoverHome."', '".$sImagePathCoverInner."')";
					//echo $qInsert;
					...
					if(mysqli_query($connEMM, $qInsert)){
						...
						VALUES('".$_SESSION["sessUserName"]."', 1, 'en_bas_specialcategory', '".$_SERVER["REMOTE_ADDR"]."', '".$_SERVER["REQUEST_URI"]."', '".fAuTrail($qInsert)."', '".$dtDateTime."')";
								...
								while($fileRen=readdir($dir)){
									if($fileRen==$sImagePathIcon){
										...
										if($iFlag==1){
											...
											$qUpdate="UPDATE en_bas_specialcategory SET ImagePathIcon='".$sNewName."' WHERE SpecialCategoryID=".$iThisContentID;
											//echo $qUpdate."<br>";
							...
							if(move_uploaded_file($_FILES["txtImageMenu"]["tmp_name"], $UploadCategory.$_FILES["txtImageMenu"]["name"])){
								...
								while($fileRen=readdir($dir)){
									if($fileRen==$sImagePathMenu){
										...
										if($iFlag==1){
											//If Rename was done properly -- Update the new uploaded file name information into the database
											$qUpdate="UPDATE en_bas_specialcategory SET ImagePathMenu='".$sNewName."' WHERE SpecialCategoryID=".$iThisContentID;
											//echo $qUpdate."<br>";
							...
							if(move_uploaded_file($_FILES["txtImageCoverHome"]["tmp_name"], $UploadCategory.$_FILES["txtImageCoverHome"]["name"])){
								...
								while($fileRen=readdir($dir)){
									if($fileRen==$sImagePathCoverHome){
										...
										if($iFlag==1){
											//If Rename was done properly -- Update the new uploaded file name information into the database
											$qUpdate="UPDATE en_bas_specialcategory SET ImagePathCoverHome='".$sNewName."' WHERE SpecialCategoryID=".$iThisContentID;
											//echo $qUpdate."<br>";
							...
							if(move_uploaded_file($_FILES["txtImageCoverInner"]["tmp_name"], $UploadCategory.$_FILES["txtImageCoverInner"]["name"])){
								...
								while($fileRen=readdir($dir)){
									if($fileRen==$sImagePathCoverInner){
										...
										if($iFlag==1){
											//If Rename was done properly -- Update the new uploaded file name information into the database
											$qUpdate="UPDATE en_bas_specialcategory SET ImagePathCoverInner='".$sNewName."' WHERE SpecialCategoryID=".$iThisContentID;
											//echo $qUpdate."<br>";
						...
						echo $sMsgInsert;
						header("Location: speCatInsert.php");
					}else{
