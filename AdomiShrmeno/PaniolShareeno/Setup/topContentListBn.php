...
<div class="page-title"><h3 class="title">Dashboard / Top Content List (Bangla)</h3></div>
	...
	<div class="col-sm-12 DPaddingLR">
		...
		<div class="panel panel-default">
			...
			<div class="panel-heading">
				...
				<h3 class="panel-title">Content - Top Content List (Bangla)</h3>
				...
				<div id="steps-uid-0" class="wizard clearfix" role="application">
							...
							<div class="form-group clearfix">
								...
								<div class="col-sm-2">
									...
									<select class="form-control" onchange="navigateSubCategory(this.value)" id="exampleFormControlSelect1">
										...
										<?php $resultSubCategory=mysqli_query($connEMM, "SELECT * FROM bn_bas_subcategory WHERE Deletable=1 ORDER BY SubCategoryName ASC") or die(mysqli_error($connEMM));
								...
								<div class="col-sm-2">
									...
									<select class="form-control" onchange="navigateSpeCategory(this.value)" id="exampleFormControlSelect1">
										...
										<?php $resultSpecialCategory=mysqli_query($connEMM, "SELECT * FROM bn_bas_specialcategory WHERE Deletable=1 ORDER BY SpecialCategoryName ASC") or die(mysqli_error($connEMM));
									...
									<table class="table table-hover table-bordered text-left">
										...
										<tbody>
											...
											elseif($iSpeCategoryID>0){
												...
												$qContent="SELECT bn_content.*, bn_bas_category.CategoryName, bn_bas_category.Slug, bn_bas_subcategory.SubCategoryName, bn_bas_specialcategory.SpecialCategoryName,  bn_totalhit.TotalHit FROM bn_content INNER JOIN bn_bas_category ON bn_content.CategoryID=bn_bas_category.CategoryID INNER JOIN bn_bas_subcategory ON bn_content.SubCategoryID=bn_bas_subcategory.SubCategoryID INNER JOIN bn_bas_specialcategory ON bn_bas_specialcategory.SpecialCategoryID=bn_content.SpecialCategoryID INNER JOIN bn_totalhit ON bn_totalhit.ContentID=bn_content.ContentID WHERE bn_content.Deletable=1 AND bn_content.SpecialCategoryID=".$iSpeCategoryID." ORDER BY bn_content.ContentID DESC LIMIT $offset, $rowsPerPage";
					...
					<div class="actions clearfix DMarginTop20">
						...
						<ul role="menu" aria-label="Pagination">
							...
							<?php if($iCategoryID>0){$qCoutner="SELECT COUNT(ContentID) AS numrows FROM bn_content WHERE CategoryID=".$iCategoryID;
...
<script type="text/javascript">
	...
	function navigateCategory(id) {
		...
		window.location.assign("topContentListBn.php?catid="+id+"&page=1")

	}

	function navigateSubCategory(id) {

		window.location.assign("topContentListBn.php?subcatid="+id+"&page=1")

	}

	function navigateSpeCategory(id) {

		window.location.assign("topContentListBn.php?spcatid="+id+"&page=1")
